...
class ProcessoBebidas extends Component {
  constructor(props) {
    ...
    if (inProgresso.cocktails) {
      if (!inProgresso.cocktails[params.id]) {
        localStorage.setItem(
          'inProgressRecipes',
          JSON.stringify({
            ...inProgresso,
            cocktails: { ...inProgresso.cocktails, [params.id]: [] },
          }),
        );
      }
    } else {
    ...
    const pro2 = pro.cocktails[params.id];
    this.state = {
      receita: {},
      favorito: false,
      mensagem: null,
  ...
  lista() {
    ...
    const { match: { params } } = this.props;
    const inProgresso = JSON.parse(localStorage.getItem('inProgressRecipes'));
    const list = inProgresso.cocktails[params.id];
